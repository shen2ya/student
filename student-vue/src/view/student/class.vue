      ...
      <Row>
        <Col span="4">
          <FormItem label="Class">
            <Input v-model="formSearch.name" placeholder="input class"></Input>
          </FormItem>
        </Col>
        <Col span="6">
    ...
    <Modal v-model="addFlag" width="400">
      ...
      <div style="text-align:center;width:90%">
        <Form :model="formAdd" :label-width="90">
          <FormItem label="Class">
            <Input v-model="formAdd.name" placeholder="input class"/>
          </FormItem>
        </Form>
      </div>
      <div slot="footer">
...
export default {
  ...
  methods: {
    search (index) {
      ...
      this.$ajax.post('class/getList', this.formSearch).then(response => {
    ...
    doUpdate (param) {
      this.$ajax.post('class/update', param).then(response => {
        let data = response.data
        if (data.success) {
    ...
    doAdd () {
      ...
      this.$ajax.post('class/add', this.formAdd).then(response => {
        let data = response.data
        if (data.success) {
