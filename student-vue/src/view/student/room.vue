  ...
  <div>
    <Form :model="formSearch" :label-width="80">
      <Row>
        <Col span="4">
          <FormItem label="Dorm">
            <Input v-model="formSearch.name" placeholder="input dorm"></Input>
          </FormItem>
        </Col>
        <Col span="6">
    ...
    <Modal v-model="addFlag" width="400">
      <p slot="header">
        <span v-if="formAdd.id">Updaet</span>
        <span v-else>Add</span>
      </p>
      <div style="text-align:center;width:90%">
        <Form :model="formAdd" :label-width="90">
          <FormItem label="Dorm">
            <Input v-model="formAdd.name" placeholder="input dorm"/>
          </FormItem>
          <FormItem label="Building">
            <Select v-model="formAdd.buildId" placeholder="select building">
...
export default {
  ...
  methods: {
    search (index) {
      ...
      this.$ajax.post('buildRoom/getList', this.formSearch).then(response => {
    ...
    doUpdate (param) {
      this.$ajax.post('buildRoom/update', param).then(response => {
        let data = response.data
        if (data.success) {
    ...
    doAdd () {
      ...
      this.$ajax.post('buildRoom/add', this.formAdd).then(response => {
        let data = response.data
        if (data.success) {
